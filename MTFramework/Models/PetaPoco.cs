            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        cmd.CommandText += ";\nSELECT SCOPE_IDENTITY() AS NewID;";//Change by Mal on 2017-3-30 for a trigger bug
                    ...
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                                ...
                                if (i.Value.ResultColumn)
                                    continue;
                                //BUG 当属性为NULL仍然进行UPDATE 已修改 BY 张朋涛
                                if (null == i.Value.GetValue(poco))
                                {
                                    continue;
                                }
    ...
    */
        ...
        static class AutoSelectHelper
        {
            public static string AddSelectClause<T>(DatabaseType DatabaseType, string sql)
            {
                ...
                if (!rxSelect.IsMatch(sql))
                {
                    ...
                    if (DatabaseType.GetType() == typeof(OracleDatabaseType))
                    {
                    ...
                    if (DatabaseType.GetType() == typeof(OracleDatabaseType))
                    {
                        cols = pd.Columns.Count != 0 ? string.Join(", ", (from c in pd.QueryColumns select tableName + "." + DatabaseType.EscapeSqlIdentifier(c)).ToArray()) : "NULL";
                    }
    ...
    namespace DatabaseTypes
    {
        ...
        class OracleDatabaseType : DatabaseType
        {
            ...
            public override string BuildPageQuery(long skip, long take, PagingHelper.SQLParts parts, ref object[] args)
            {
                ...
                var sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                                           parts.sqlOrderBy == null ? "ORDER BY (ID)" : parts.sqlOrderBy, parts.sqlSelectRemoved, args.Length, args.Length + 1);
        ...
        class SqlServerDatabaseType : DatabaseType
        {
            ...
            {
                //return String.Format(" OUTPUT INSERTED.[{0}]", primaryKeyName);//Change by Mal on 2017-3-30 for a trigger bug
                return "";//Change by Mal on 2017-3-30 for a trigger bug
            }
        }
